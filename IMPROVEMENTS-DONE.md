...
# Audit round 6 (Sessions 31–34)

An end-to-end pass driven by three parallel code-audit agents (concurrency,
security, frontend/persistence), then distilled by manual verification against
source. Round 6 is deliberately small: the codebase is already heavily
hardened, so most findings are residuals, not regressions.
...
All items in Sessions 31–34 were confirmed directly against the current source
(file:line, behaviour) during this audit — the reported bug is real, not a false
positive. Session 34's verify-first items were each confirmed before fixing.
...
Surfaced by an audit agent. Each item was confirmed against source
(file:line + behaviour) before fixing. These are independent,
low-to-medium severity, and each is a contained edit.

- [x] **F31** [Medium, verified+fixed] Confirm-dialog Escape leaks the
  in-flight promise. `frontend/shared/ui.js:157` registered the dialog
  key handler lazily on first open (bubble phase); the app handler
  (`frontend/src/formateur/renderers.js:1067`, registered at init via
  `main.js`) registered earlier, so it won the dispatch race. On
  Escape-while-confirm-open, the app handler synchronously called
  `showConfirmDialog`, whose Promise constructor overwrote
  `confirmResolve` — dropping the original click handler's resolver.
  The dialog handler then resolved the *app* handler's promise; the
  click handler's `await` never resolved. **Fix:** (1) register the
  dialog key handler in the **capture phase** (`addEventListener(..., true)`)
  so it fires before any init-time bubble handler regardless of
  registration order; `stopPropagation` + `stopImmediatePropagation`
  prevent the event from reaching the bubble phase. (2) Guard the app
  handler with `isConfirmDialogOpen()` (exported from `ui.js`) so the
  AT_TARGET edge case (event dispatched directly on `document`) is also
  covered.
- [x] **F32** [Medium, verified+fixed] `saveHighScore` /
  `setLastConfig` swallowed localStorage quota failures → silent data
  loss framed as success. `saveHighScore` returned `false` on a quota
  throw indistinguishably from a non-record; `setLastConfig` returned
  void and silently dropped the config. **Fix:** `saveHighScore` now
  returns `{ isRecord, persisted }` — `isRecord` is `true` even when
  persistence failed so the "Nouveau record !" badge still shows, while
  a `saveFailed` flag on the game state drives an info toast
  (`gameSaveFailed`). `setLastConfig` returns `boolean`; the formateur
  `startVote` handler surfaces `configSaveFailed` on failure.
- [x] **R25** [Low, verified+fixed] Data race on `Client.ID` / `Name` /
  `Type` / `SessionID` (read by `logAttrs` from writePump and
  BroadcastSession fanout goroutines, written in readPump). Go string
  assignment is a two-word write, not atomic. Confirmed under `-race`
  by reverting the fix and observing the DATA RACE report. **Fix:**
  added `atomic.Pointer[clientIdentity]` on `Client`, snapshotted via
  `snapshotIdentity()` after every identity mutation (join handlers +
  update_name). `logAttrs` and `trySend`'s role check read the atomic
  snapshot. `identitySnapshot()` is nil-safe (fallback to raw fields)
  for test fixtures that bypass `NewClient`. `NewClient` now takes
  `clientID` so the initial snapshot is populated before `Start()`.
- [x] **R26** [Low, verified+fixed] TOCTOU window in `cleanupLoop` on
  the `protected` snapshot. `hub.go` snapshotted `protected` under
  `h.mu.RLock`, released the lock, then called
  `CleanupExpiredSessions(..., protected)`. A session published in the
  window was not in `protected` and could be reaped if its
  `LastActivity` read as stale. **Fix:** hold `h.mu.RLock` across the
  `CleanupExpiredSessions` call (lock order `h.mu → m.mu → session.mu`
  is the established invariant, so nesting is safe). Cleanup runs at
  `CleanupInterval`, not on the hot path, so the brief serialization
  with register is acceptable.
- [x] Tests: `shared/ui.test.js` (+F31 capture-phase intercepts a
  pre-registered bubble handler; +updated comment on the existing
  stopImmediatePropagation test), `formateur/renderers.test.js`
  (+F31 app handler skips when dialog is open), `game-storage.test.js`
  (+F32 saveHighScore returns `{ isRecord: true, persisted: false }` on
  quota throw; updated all saveHighScore tests for the new return
  shape), `presets.test.js` (+F32 setLastConfig returns false on quota;
  +returns true on success), `hub/client_test.go`
  (+`TestClientIdentityAtomicityRace` — concurrent ID write + logAttrs
  read under `-race`, confirmed the race without the fix),
  `hub/hub_test.go` (+`TestCleanupLoopAtomicProtectedReap` — 10
  sessions registered concurrently with a tight cleanup loop at
  `SessionTimeout=1ns` all survive). `go test -race ./...` green;
  `make fmt-check` / `go vet` clean; `npm test` green (621 tests);
