...
- [ ] **CC2** TOCTOU name uniqueness — `IsNameInUse` runs in client goroutine; actual map write happens later in `Hub.Run` without re-checking. Two "jean"s both register — `backend/internal/hub/client.go:255-300`, `backend/internal/vote/manager.go:107`. Move check into `JoinStagiaire` under `session.mu.Lock`.
- [ ] **CC3** Global hub write-lock held across JSON marshal + map copies + channel send on every register/unregister — `backend/internal/hub/hub.go:174-176`, `notifyTrainerStagiaireListLocked:495-544`. O(N²) under reconnect storm.
- [ ] **CC1** Trainer send buffer overflows under burst (30 votes fan out ~60 trainer-bound msgs); `default` branch disconnects the trainer — `backend/internal/hub/client.go:158-164`. Raise trainer buffer or use blocking send-with-timeout for the trainer path.
- [ ] **CM3** `BroadcastSession` slow-client `default` branch closes the conn but leaves it in the map → warning spam until `pongWait` evicts — `backend/internal/hub/hub.go:415-422`.
- [ ] **CL1** Old client's `Send` channel never drained after reconnect-by-ID takeover — `backend/internal/hub/hub.go:301-306`. Add `closing` flag, short-circuit `SendJSON`.
- [ ] Stress test: simulate 30-client reconnect storm under `-race`.
